...
class fMRI_LSTM(nn.Module):
    ...
    def init_hidden(self, batch_size):
        return (
        torch.randn(1, batch_size, self.hidden_dim).to(device), torch.randn(1, batch_size, self.hidden_dim).to(device))
...
# def train(training_data, training_labels, training_sample_names, testing_data, testing_labels, testing_sample_names,
# num_timesteps):
...
#                 model.train()
#                 training_sample = training_data[i][:, np.random.choice(training_data[i].shape[1], num_timesteps,
#                 replace=False)]
...
#     num_timesteps = 25
#     train(training_data, training_labels, training_sample_names, testing_data, testing_labels, testing_samples_names
#     , num_timesteps)
